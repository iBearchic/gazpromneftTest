...
url = f"https://api.gazprombank.ru/very/important/docs?documents_date={int(start_of_today)}"
response = requests.get(url)
data = response.json()

try:
    document_data = DocumentData(**data)
except ValidationError as e:
    print("Ошибка валидации данных:", e)
    raise

rows = [{col: val for col, val in zip(data["Columns"], row)} for row in data["Rows"]]
